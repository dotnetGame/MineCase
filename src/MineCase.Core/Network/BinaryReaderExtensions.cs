...
namespace MineCase.Network
{
    internal static class BinaryReaderExtensions
    {
        ...
        public static long[] ReadAsLongArray(this BinaryReader br)
        {
            int length = br.ReadAsVarInt(out _);
            long[] array = new long[length];

            for (int i = 0; i < array.Length; ++i)
            {
                array[i] = br.ReadAsLong();
            }

            return array;
        }

        public static ulong[] ReadAsUnsignedLongArray(this BinaryReader br)
        {
            int length = br.ReadAsVarInt(out _);
